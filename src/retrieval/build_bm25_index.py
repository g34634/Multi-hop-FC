...
# 제목과 문단 텍스트를 BM25 검색용 본문 문자열로 결합한다.
def _build_bm25_text(title: str, paragraph_text: str) -> str:
...
# 단일 parquet 파일을 BM25 청크 리스트 형식으로 변환한다.
        ...
        chunks.append(
            {
                ...
                "text": _build_bm25_text(row.title, row.paragraph_text),
                "metadata": {
                    "doc_id": row.doc_id,
                    "title": row.title,
...
# 디렉터리 아래 parquet 파일 목록을 재귀 탐색해 정렬 반환한다.
...
# parquet 디렉터리의 모든 파일을 읽어 하나의 청크 리스트로 합친다.
...
def iter_parquet_dir_chunk_batches(
    ...
    """
    로딩 시 메모리 피크를 줄이기 위해 파일 배치 단위 청크 리스트를 생성한다.
    """
    ...
    file_indices = range(0, total_files, files_per_batch)
    if use_tqdm and tqdm is not None:
        file_indices = tqdm(
            file_indices,
            total=(total_files + files_per_batch - 1) // files_per_batch,
            desc="BM25 batches",
            unit="batch",
        )
    ...
    for start in file_indices:
        ...
        for parquet_path in batch_files:
            chunks = load_parquet_as_chunks(parquet_path)
...
# 청크 리스트를 pyserini 입력 형식 JSONL 파일로 저장한다.
...
# 단일 parquet 파일에서 BM25 인덱스를 직접 생성한다.
...
# parquet 디렉터리를 배치로 변환해 단일 Lucene BM25 인덱스를 생성한다.
def build_bm25_index_from_parquet_dir_batched(
    ...
    """
    parquet 샤드를 배치 JSONL 파일로 변환한 뒤,
    해당 디렉터리에서 단일 Lucene BM25 인덱스를 생성한다.
    전체 청크를 한 번에 메모리에 올리지 않기 위한 방식이다.
    """
    print(f"Loading parquet directory in batches: {parquet_dir}")
    print(f"files_per_batch: {files_per_batch}")
    print(f"bm25_threads: {threads}")
    ...
    config.bm25_expected_docs = total_chunks
...
# 이미 생성된 JSONL 샤드 디렉터리에서 BM25 인덱스를 생성한다.
def build_bm25_index_from_jsonl_tmp_dir(
    ...
    """
    기존 JSONL 샤드 디렉터리에서 Lucene BM25 인덱스를 직접 생성한다.
    입력 디렉터리 예시:
      data/interim/bm25/bm25_index_all_shards_lucene_jsonl_tmp
      (part_00001.jsonl, ..., part_00122.jsonl 포함)
    """
